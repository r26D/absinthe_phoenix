defmodule Absinthe.Phoenix.Channel do
  ...
  def join("__absinthe__:control", _, socket) do
    ...
    pubsub_ops_module = socket.assigns[:__pubsub_ops_module__]
    ...
    absinthe_config = absinthe_config
                      |> Map.put(:pipeline, pipeline || {__MODULE__, :default_pipeline})
                      |> Map.put(:pubsub_ops_module, pubsub_ops_module)
  ...
  def handle_in("unsubscribe", %{"subscriptionId" => doc_id}, socket) do
    pubsub_ops_module = socket.assigns[:absinthe]
                        |> Map.get(:pubsub_ops_module)
    pubsub =
      socket.assigns
      |> Map.get(:absinthe, %{})
      |> Map.get(:opts, [])
    ...
    apply(pubsub_ops_module, :unsubscribe, [socket.pubsub_server, doc_id])
